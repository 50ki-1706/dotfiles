...
20260831 20:05:49 +0900 - OpenCodeのGraphify MCP起動依存を`graphifyy[mcp]`へ変更し、MCPサーバーに必要なPythonパッケージを導入しました。
20260831 20:21:29 +0900 - OpenCodeの全ローカルMCP実行ファイルをNix storeの絶対パスで生成し、ブラウザ系MCPにはNode.jsとシェルのPATHを明示して、Home Manager activationから再起動したサービスでも接続できるようにしました。
20260831 22:01:29 +0900 - OpenCode V2を廃止し、programs.opencodeをnixpkgsのpkgs.opencodeへ復元しました。opencode2互換ラッパーとV2サービス再起動hookを削除し、plan-review-read-scope.jsの配置を廃止しました。
