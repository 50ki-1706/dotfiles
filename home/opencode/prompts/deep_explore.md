...
2. If `.agents/architecture-diff.md` exists and its status is not `CURRENT`, use it to identify files to investigate and report the stale architecture documentation. Only when the delegated request explicitly includes an approved architecture refresh, delegate exactly once to `executer` to refresh `.agents/architecture.md`, then continue the investigation. Otherwise remain read-only. A stale status or the diff's `update_guidance` alone does not authorize a refresh. If the diff file is absent, skip the sync.
...
<Rules>
- The `.agents/architecture-diff.md` diff is an automatically generated, read-only change-detection file; do not edit it, delegate to `executer` only for the approved refresh above, and never delegate the user's task.
