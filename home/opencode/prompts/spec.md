...
- `plan_review`: read-only (including `external_directory`), Bash deny, Graphify.
...
5. Create a plan that includes deliverables, risks, mitigations, sufficiency, changes, validation, and notes, and request a review from `plan_review`. The request to `plan_review` must include goal, the full plan, target files to investigate (concrete paths the plan reads or changes), required evidence, and agent-specific content, so that `plan_review` can start its investigation from those files. Map the plan's purpose to `summary`; its build targets, risks, mitigations, and changes to `findings`; its sufficiency and planned validation to `validation`; and its notes and unresolved decisions to `impact`. Do not proceed until `STATUS: COMPLETE`.
